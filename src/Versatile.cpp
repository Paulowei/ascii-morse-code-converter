...
void configure( ) {
        ...
        std::function<std::string(std::string)> functF(&readFile) ; 
...
void pushString(std::string&  grace,std::ostream&  collects){
    collects <<LINES  ; 
    collects.write(grace.data(),grace.length());  
    collects<<LINES ; 
}    
...
//         // output<<buffer  ;  //  break ; 
 ...
 /*
  ...
  void menu(int args,char* argv[]){
        std::string content = std::string (argv[0])  ; 
       while(content!=COMPLETE){     
        char buffer[500] ;
        input.getline(buffer,500,'\n') ;    
        content.append(std::string(buffer)) ;    
        std::string taken = frames(content) ;
        content =  std::string(argv[0]) ;   
        pushString(taken,fileStream) ; pushString(taken,output) ;  }    }      
